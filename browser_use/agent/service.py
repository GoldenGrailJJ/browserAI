...
def log_response(response: AgentOutput) -> None:
	...
	try:
		...
		# 创建日志目录
		log_dir = Path("logs/json_logs")
		log_dir.mkdir(parents=True, exist_ok=True)

		# 按日期分割文件
		log_file = log_dir / f"agent_{datetime.now().strftime('%Y%m%d')}.json"
		...
		logger.error(f"JSON序列化失败: {str(e)}", exc_info=True)
...
class Agent(Generic[Context]):
	...
	async def step(self, step_info: Optional[AgentStepInfo] = None) -> None:
		...
		try:
			...
			try:
				...
				if self.settings.save_conversation_path:
					target = self.settings.save_conversation_path + f'_{self.state.n_steps}_steps.txt'
	...
	def _convert_input_messages(self, input_messages: list[BaseMessage]) -> list[BaseMessage]:
		...
		if self.model_name == 'deepseek-reasoner' or 'deepseek-r1' in self.model_name:
			...
			processed_messages = input_messages  # 保持原有逻辑

			try:
				# 创建日志目录
				log_dir = Path("logs/agent_io")
				log_dir.mkdir(parents=True, exist_ok=True)
				...
				metadata = {
					...
					"input_type": type(input_messages).__name__
				}

				# 结构化存储
				dump_data = {
					"metadata": metadata,
					"raw_input": [msg.model_dump() for msg in input_messages],  # 假设使用Pydantic模型
					"processed_input": [msg.model_dump() for msg in processed_messages]
				}

				# 写入文件（原子操作）
				filename = log_dir / f"input_{datetime.now().strftime('%Y%m%d%H%M%S')}.json"
				with open(filename, "w", encoding="utf-8") as f:
					json.dump(dump_data, f, indent=2, ensure_ascii=False)

			except Exception as e:
				print(f"输入消息存储失败: {str(e)}")
		...
		if self.tool_calling_method == 'raw':
			...
			try:
				...
				metadata = {
					...
					"execution_time": time_execution_async.latest_time  # 假设有计时器
				}

				# 结构化输出数据
				dump_data = {
					"metadata": metadata,
					"input_hash": hash(str(input_messages)),  # 建立输入输出关联
					"raw_output": output.model_dump() if hasattr(output, "model_dump") else str(output),
					"actions": [
						action.model_dump(exclude_unset=True)
						for action in output.action
					] if hasattr(output, "action") else []
				}

				# 写入文件
				filename = Path("logs/agent_io") / f"output_{datetime.now().strftime('%Y%m%d%H%M%S')}.json"
				with open(filename, "w", encoding="utf-8") as f:
					json.dump(dump_data, f, indent=2, ensure_ascii=False)

			except Exception as e:
				print(f"输出结果存储失败: {str(e)}")
	...
	async def run(self, max_steps: int = 6) -> AgentHistoryList:
